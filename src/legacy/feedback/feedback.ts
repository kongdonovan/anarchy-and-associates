...
const logger = new Logger("FeedbackCommand");
...
export class FeedbackCommand {
   ...
   * Submit feedback to the admins, optionally ping a user
   */
  @Slash({ name: "feedback", description: "Submit feedback to the admins" })
  async feedback(
    ...
    try {
      ...
      // Send feedback to a configurable channel
  ...
  /**
   * /searchfeedback [user] [from] [to]
   * Admin: Search feedback by user or date
   */
  @Slash({ name: "searchfeedback", description: "Admin: Search feedback by user or date" })
  async searchFeedback(
    ...
    try {
      // Only allow admins (add your own permission check if needed)
