...
export class RepairService {
  ...
  async repairStaffRoles(guild: Guild, dryRun = false): Promise<RepairResult> {
    ...
    try {
      ...
      for (const staff of staffMembers) {
        ...
        const expectedRole = guild.roles.cache.find(role => 
          role.name === staff.role || role.name.toLowerCase().includes(staff.role.toLowerCase())
        );
      ...
      logger.error('Error in staff roles repair:', error);
      return {
        success: false,
  ...
  async repairJobRoles(guild: Guild, dryRun = false): Promise<RepairResult> {
    const changes: string[] = [];
    const errors: string[] = [];

    try {
      logger.info(`Starting job roles repair for guild ${guild.id}, dry-run: ${dryRun}`);
      ...
      logger.error('Error in job roles repair:', error);
      return {
        success: false,
  ...
  async repairChannels(guild: Guild, dryRun = false): Promise<RepairResult> {
    const changes: string[] = [];
    const errors: string[] = [];

    try {
      logger.info(`Starting channels repair for guild ${guild.id}, dry-run: ${dryRun}`);

      const config = await this.guildConfigRepository.ensureGuildConfig(guild.id);
      const requiredChannels = [
        { key: 'feedbackChannelId', name: 'feedback', type: 'text' as const },
        { key: 'retainerChannelId', name: 'retainer-agreements', type: 'text' as const },
        { key: 'modlogChannelId', name: 'mod-log', type: 'text' as const },
        { key: 'applicationChannelId', name: 'job-applications', type: 'text' as const },
      ];
      ...
      for (const channel of requiredChannels) {
        const channelId = config[channel.key as keyof typeof config] as string;
      ...
      for (const category of requiredCategories) {
        const categoryId = config[category.key as keyof typeof config] as string;
      ...
      logger.error('Error in channels repair:', error);
      return {
        success: false,
  ...
  async repairConfig(guild: Guild, dryRun = false): Promise<RepairResult> {
    const changes: string[] = [];
    const errors: string[] = [];

    try {
      logger.info(`Starting config repair for guild ${guild.id}, dry-run: ${dryRun}`);
      ...
      for (const action of permissionActions) {
        const roleIds = config.permissions[action as keyof typeof config.permissions];
        for (const roleId of roleIds) {
          ...
          if (!role) {
            ...
            if (!dryRun) {
              ...
              permissions[action as keyof typeof permissions] = updatedRoles;
      ...
      logger.error('Error in config repair:', error);
      return {
        success: false,
  ...
  async repairOrphaned(guild: Guild, dryRun = false): Promise<RepairResult> {
    const changes: string[] = [];
    const errors: string[] = [];

    try {
      logger.info(`Starting orphaned records repair for guild ${guild.id}, dry-run: ${dryRun}`);
      ...
      logger.error('Error in orphaned records repair:', error);
      return {
        success: false,
  ...
  async repairDbIndexes(guild: Guild, dryRun = false): Promise<RepairResult> {
    const changes: string[] = [];
    const errors: string[] = [];

    try {
      logger.info(`Starting database indexes repair for guild ${guild.id}, dry-run: ${dryRun}`);
      ...
      logger.error('Error in database indexes repair:', error);
      return {
        success: false,
  ...
  async repairAll(guild: Guild, dryRun = false): Promise<RepairResult> {
    const allChanges: string[] = [];
    const allErrors: string[] = [];
    const results: RepairResult[] = [];

    try {
      logger.info(`Starting comprehensive repair for guild ${guild.id}, dry-run: ${dryRun}`);
      ...
      logger.error('Error in comprehensive repair:', error);
      return {
        success: false,
  ...
  async performHealthCheck(guild: Guild): Promise<HealthCheckResult> {
    const issues: string[] = [];
    const checks = {
      database: false,
      channels: false,
      ...
      try {
        ...
        if (botMember) {
          ...
          const missingPermissions = requiredPermissions.filter(perm => 
            !botMember.permissions.has(perm as any)
          );
