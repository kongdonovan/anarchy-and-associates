...
describe('Job and Application Entities', () => {
  describe('Job Entity', () => {
    describe('Job Creation and Validation', () => {
      it('should create a valid job with required fields', () => {
        const job = TestUtils.generateMockJob();
        
        expect(job.guildId).toBeTruthy();
        expect(job.title).toBeTruthy();
        expect(job.description).toBeTruthy();
        expect(job.staffRole).toBe(StaffRole.PARALEGAL);
      ...
      it('should handle all valid staff roles', () => {
        ...
        validRoles.forEach(role => {
          const job = TestUtils.generateMockJob({ staffRole: role });
      ...
      it('should track job status correctly', () => {
        const openJob = TestUtils.generateMockJob({ isOpen: true });
        const closedJob = TestUtils.generateMockJob({ 
          isOpen: false,
          closedAt: new Date(),
          closedBy: 'hr123'
        });
      ...
      it('should handle position limits', () => {
        const limitedJob = TestUtils.generateMockJob({ 
    ...
    describe('Job Questions Management', () => {
      it('should handle default job questions', () => {
        expect(DEFAULT_JOB_QUESTIONS).toBeDefined();
        expect(Array.isArray(DEFAULT_JOB_QUESTIONS)).toBe(true);
        expect(DEFAULT_JOB_QUESTIONS.length).toBeGreaterThan(0);
        
        // Verify required questions
        const requiredQuestions = DEFAULT_JOB_QUESTIONS.filter(q => q.required);
        expect(requiredQuestions.length).toBeGreaterThan(0);
        
        // Verify question types
        const questionTypes = DEFAULT_JOB_QUESTIONS.map(q => q.type);
      ...
      it('should validate question type constraints', () => {
        const questionsWithConstraints: JobQuestion[] = [
          {
            id: 'short_text',
      ...
      it('should handle questions with no constraints', () => {
        const unconstrainedQuestion: JobQuestion = {
          id: 'open_ended',
    ...
    describe('Job Statistics Tracking', () => {
      ...
      it('should handle zero counts', () => {
        const newJob = TestUtils.generateMockJob({
          applicationCount: 0,
          hiredCount: 0
        });
  ...
  describe('Application Entity', () => {
    describe('Application Creation and Validation', () => {
      ...
      it('should handle all valid application statuses', () => {
        ...
        validStatuses.forEach(status => {
          const application = TestUtils.generateMockApplication({ status });
      ...
      it('should track review information', () => {
        ...
        const reviewedApplication = TestUtils.generateMockApplication({
          status: 'accepted',
          reviewedBy,
    ...
    describe('Application Answers Management', () => {
      ...
      it('should handle answers with special characters', () => {
        const specialAnswers: ApplicationAnswer[] = [
          {
    ...
    describe('Application Review Process', () => {
      it('should handle application acceptance', () => {
        ...
        const acceptedApplication = TestUtils.generateMockApplication({
          status: 'accepted',
      ...
      it('should handle application rejection', () => {
        ...
        const rejectedApplication = TestUtils.generateMockApplication({
          status: 'rejected',
          reviewedBy: 'hr_manager',
  ...
  describe('Cross-Guild Isolation', () => {
    it('should maintain separate job postings per guild', () => {
      const guild1Job = TestUtils.generateMockJob({
        guildId: 'guild1',
        title: 'Corporate Lawyer',
        staffRole: StaffRole.JUNIOR_ASSOCIATE
      });

      const guild2Job = TestUtils.generateMockJob({
        guildId: 'guild2',
        title: 'Corporate Lawyer', // Same title, different guild
        staffRole: StaffRole.SENIOR_ASSOCIATE // Different requirements
      });
  ...
  describe('Edge Cases and Validation', () => {
    it('should handle jobs with no questions', () => {
      const job = TestUtils.generateMockJob({ questions: [] });
    ...
    it('should handle applications with missing optional fields', () => {
      const minimalApplication = TestUtils.generateMockApplication({
    ...
    it('should handle special characters in job titles and descriptions', () => {
      const specialJob = TestUtils.generateMockJob({
    ...
    it('should handle concurrent application submissions', () => {
      const simultaneousApplications = Array.from({ length: 5 }, (_, i) => 
        TestUtils.generateMockApplication({
          applicantId: `user${i}`,
          createdAt: new Date(),
          updatedAt: new Date()
