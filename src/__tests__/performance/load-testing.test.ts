...
describe('Performance and Load Testing', () => {
  ...
  describe('Bulk Operation Performance', () => {
    it('should handle bulk staff hiring efficiently', async () => {
      ...
      const hirePromises = Array.from({ length: bulkSize }, (_, i) =>
    ...
    it('should handle bulk case creation efficiently', async () => {
      ...
      const casePromises = Array.from({ length: bulkSize }, (_, i) =>
  ...
  describe('Search and Query Performance', () => {
    ...
    it('should perform filtered searches efficiently', async () => {
      const searchTests = [
        () => caseService.searchCases({ guildId: testGuildId, priority: CasePriority.HIGH }),
        () => caseService.searchCases({ guildId: testGuildId, status: CaseStatus.PENDING }),
        () => staffService.getStaffList(testGuildId, adminUserId, StaffRole.PARALEGAL),
  ...
  describe('Concurrent Load Testing', () => {
    ...
    it('should maintain performance under sustained load', async () => {
      ...
      const sustainedLoadInterval = setInterval(() => {
        operationCount++;
        operations.push(
          staffService.getStaffList(testGuildId, `sustained-${operationCount}`)
        );
      }, operationInterval);
  ...
  describe('Scalability Testing', () => {
    it('should scale with increasing data volume', async () => {
      ...
      for (const dataSize of scalingTests) {
        ...
        // Create test data
        const setupPromises = Array.from({ length: dataSize }, (_, i) =>
          caseService.createCase({
            guildId: testGuildId,
        ...
        const maxAcceptableDuration = Math.min(
          PERFORMANCE_THRESHOLDS.SEARCH_OPERATION_MS * (dataSize / 10),
    ...
    it('should handle queue scaling efficiently', async () => {
      ...
      for (const queueSize of queueSizes) {
        ...
        // Fill queue
        const queuedOperations = Array.from({ length: queueSize }, (_, i) =>
          operationQueue.enqueue(
            () => Promise.resolve(`queued-operation-${i}`),
            `user-${i}`,
            testGuildId,
            false
          )
  ...
  describe.skip('Memory and Resource Usage', () => {
    it('should maintain reasonable memory usage during bulk operations', async () => {
      ...
      const bulkOperations = Array.from({ length: 1000 }, (_, i) =>
        caseService.createCase({
          guildId: testGuildId,
    ...
    it('should handle resource cleanup efficiently', async () => {
      ...
      for (let cycle = 0; cycle < 5; cycle++) {
        const cycleOperations = Array.from({ length: 100 }, (_, i) =>
          staffService.getStaffList(testGuildId, `cleanup-test-${cycle}-${i}`)
        );
  ...
  describe('Database Query Optimization', () => {
    it('should perform efficient database queries', async () => {
      ...
      const queryTests = [
        ...
        () => staffRepository.findByRole(testGuildId, StaffRole.PARALEGAL),
