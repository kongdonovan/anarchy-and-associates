...
describe('Discord Command Workflows E2E Tests', () => {
  ...
  describe('Staff Management Workflow', () => {
    it('should complete full staff hiring workflow with permissions', async () => {
      ...
      const hireResult = await operationQueue.enqueue(
        () => staffService.hireStaff(context, {
          guildId: testGuildId,
    ...
    it('should handle staff promotion workflow with role limits', async () => {
      ...
      const promotionResult = await operationQueue.enqueue(
        () => staffService.promoteStaff(context, {
          guildId: testGuildId,
          userId: regularUserId,
  ...
  describe('Case Management Workflow', () => {
    ...
    it('should complete full case creation and assignment workflow', async () => {
      ...
      const newCase = await operationQueue.enqueue(
        () => caseService.createCase(mockPermissionContext, {
          guildId: testGuildId,
      ...
      const acceptedCase = await operationQueue.enqueue(
        () => caseService.acceptCase(leadAttorneyContext, newCase._id!.toString()),
        regularUserId,
        testGuildId,
        false
      );
    ...
    it('should handle case closure workflow with all results', async () => {
      ...
      for (const [index, result] of results.entries()) {
        ...
        const lawyerAcceptContext: PermissionContext = {
          guildId: testGuildId,
          userId: `lawyer-${index}`,
        ...
        const closedCase = await operationQueue.enqueue(
          () => caseService.closeCase(lawyerAcceptContext, {
    ...
    it('should handle concurrent case operations safely', async () => {
      ...
      const lawyer3Context: PermissionContext = {
        guildId: testGuildId,
        userId: lawyer3,
        userRoles: ['lead-attorney-role-123', 'lawyer-role-123'],
        isGuildOwner: false
  ...
  describe('Permission Boundary Testing', () => {
    it('should enforce guild owner privileges', async () => {
      ...
      const hireResult = await operationQueue.enqueue(
        () => staffService.hireStaff(guildOwnerContext, {
          guildId: testGuildId,
  ...
  describe('Cross-Guild Isolation', () => {
    it('should maintain complete isolation between guilds', async () => {
      ...
      const adminContext1: PermissionContext = {
        guildId: guild1,
        userId: adminUserId,
        userRoles: ['admin-role-123'],
        isGuildOwner: false
      };
      const adminContext2: PermissionContext = {
        guildId: guild2,
        userId: adminUserId,
        userRoles: ['admin-role-123'],
        isGuildOwner: false
      ...
      const caseContext1: PermissionContext = {
        guildId: guild1,
        userId: adminUserId,
        userRoles: ['case-role-123'],
        isGuildOwner: false
      };
      const caseContext2: PermissionContext = {
        guildId: guild2,
        userId: adminUserId,
        userRoles: ['case-role-123'],
        isGuildOwner: false
