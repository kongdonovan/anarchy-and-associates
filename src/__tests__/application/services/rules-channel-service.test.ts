import { RulesChannelService, UpdateRulesChannelRequest } from '../../../application/services/rules-channel-service';
import { RulesChannelRepository } from '../../../infrastructure/repositories/rules-channel-repository';
import { Client, TextChannel, Message } from 'discord.js';
import { RulesChannel } from '../../../validation';
...
describe('RulesChannelService', () => {
  let rulesChannelService: RulesChannelService;
  let mockRulesChannelRepository: jest.Mocked<RulesChannelRepository>;
  ...
  beforeEach(() => {
    ...
    // Initialize mocks
    mockRulesChannelRepository = new RulesChannelRepository() as jest.Mocked<RulesChannelRepository>;
    ...
    rulesChannelService = new RulesChannelService(mockRulesChannelRepository, mockDiscordClient);
  });
  ...
  describe('generateDefaultRules', () => {
    ...
    it('should generate general rules template', () => {
      const template = RulesChannelService.generateDefaultRules('general');
  ...
  describe('updateRulesChannel', () => {
    const mockRequest: UpdateRulesChannelRequest = {
    ...
    it('should successfully update rules channel', async () => {
      const mockRulesChannel: RulesChannel = {
      ...
      (mockTextChannel.messages.fetch as jest.Mock).mockResolvedValue(mockMessage);
      mockMessage.edit.mockResolvedValue(mockMessage as any);

      await rulesChannelService.updateRulesChannel(mockRequest);

      expect(mockRulesChannelRepository.upsertByChannelId).toHaveBeenCalledWith(
        mockRequest.guildId,
        mockRequest.channelId,
    ...
    it('should create new message if none exists', async () => {
      ...
      (messagesMapWithFilter as any).filter = jest.fn().mockReturnValue(new Map());
      
      (mockTextChannel.messages.fetch as jest.Mock)
      ...
      mockTextChannel.send.mockResolvedValue(mockMessage as any);
    ...
    it('should handle channel not found error', async () => {
      (mockDiscordClient.guilds.fetch as jest.Mock).mockRejectedValue(new Error('Channel not found'));
  ...
  describe('getRulesChannel', () => {
    it('should retrieve rules channel data', async () => {
      ...
      const result = await rulesChannelService.getRulesChannel(
        mockRulesChannel.guildId,
        mockRulesChannel.channelId
      );

      expect(result).toEqual(mockRulesChannel);
      expect(mockRulesChannelRepository.findByChannelId).toHaveBeenCalledWith(
        mockRulesChannel.guildId,
        mockRulesChannel.channelId
      );
    });
  ...
  describe('deleteRulesChannel', () => {
    it('should delete rules channel record', async () => {
      ...
      const result = await rulesChannelService.deleteRulesChannel(
        '123456789012345678',
        '123456789012345678'
      );
