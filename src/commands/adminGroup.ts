...
const logger = new Logger("AdminCommandGroup");
const configDb = new GuildConfigRepository();

@Discord()
@SlashGroup({ name: "admin", description: "Admin and server management commands" })
@SlashGroup("admin")
export class AdminCommandGroup {
  /**
   * /admin add [user]
   ...
   */
  @Slash({ name: "remove", description: "Remove admin privileges from a user" })
  async remove(
   ...
   */
  @Slash({ name: "revokerole", description: "Remove admin privileges from a role" })
  async revokeRole(
   ...
   */
  @Slash({ name: "list", description: "List all current admins and admin roles" })
    ...
    try {
      ...
      for (const [configKey, mapping] of Object.entries(DEFAULT_CHANNEL_MAPPINGS)) {
        const found = guild!.channels.cache.find(
          (c) =>
            c.name.toLowerCase() === mapping.name.toLowerCase() &&
  ...
  /**
   * /admin setpermissionrole [action] [role]
  ...
  // --- Repair subcommands moved to RepairCommandGroup ---
  // See src/commands/repairGroup.ts
}
