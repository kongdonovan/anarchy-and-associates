 ...
 */
class StaffService {
    constructor(staffRepository, auditLogRepository, permissionService, validationService) {
    ...
    async hireStaff(context, request) {
        try {
            ...
            const validatedRequest = validation_1.ValidationHelpers.validateOrThrow(validation_1.StaffHireRequestSchema, request, 'Staff hire request');
    ...
    async promoteStaff(context, request) {
        try {
            // Validate input using Zod schema
            const validatedRequest = validation_1.ValidationHelpers.validateOrThrow(validation_1.StaffPromoteRequestSchema, request, 'Staff promote request');
            // Check senior-staff permission (updated from HR permission)
            const hasPermission = await this.permissionService.hasSeniorStaffPermissionWithContext(context);
            if (!hasPermission) {
                return {
            ...
            await this.auditLogRepository.logAction({
                ...
                details: {
                    ...
                    reason,
                    metadata: {
                        fromRoleLevel: staff_role_1.RoleUtils.getRoleLevel(currentRole),
                        toRoleLevel: staff_role_1.RoleUtils.getRoleLevel(newRole)
                    }
                },
                timestamp: new Date()
            });
    ...
    async demoteStaff(context, request) {
        try {
            // Validate input using Zod schema
            const validatedRequest = validation_1.ValidationHelpers.validateOrThrow(validation_1.StaffPromoteRequestSchema, request, 'Staff demote request');
            // Check senior-staff permission (updated from HR permission)
            const hasPermission = await this.permissionService.hasSeniorStaffPermissionWithContext(context);
            if (!hasPermission) {
                return {
    ...
    async fireStaff(context, request) {
        try {
            // Validate input using Zod schema
            const validatedRequest = validation_1.ValidationHelpers.validateOrThrow(validation_1.StaffFireRequestSchema, request, 'Staff fire request');
            // Check senior-staff permission (updated from HR permission)
            const hasPermission = await this.permissionService.hasSeniorStaffPermissionWithContext(context);
            if (!hasPermission) {
                return {
            ...
            await this.auditLogRepository.logAction({
                ...
                details: {
                    ...
                    metadata: {
                        ...
                        roleLevel: staff_role_1.RoleUtils.getRoleLevel(staff.role)
                    }
                },
                timestamp: new Date()
            });
