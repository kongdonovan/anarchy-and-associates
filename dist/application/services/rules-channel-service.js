...
exports.RulesChannelService = void 0;
const discord_js_1 = require("discord.js");
const logger_1 = require("../../infrastructure/logger");
class RulesChannelService {
    constructor(rulesChannelRepository, discordClient) {
        this.rulesChannelRepository = rulesChannelRepository;
        this.discordClient = discordClient;
    }
    /**
     * Generates default rules template for different contexts
     */
    static generateDefaultRules(context = 'general') {
        if (context === 'anarchy') {
            return {
    ...
    async updateRulesChannel(request) {
        ...
        if (existing?.messageId) {
            ...
            try {
                ...
                logger_1.logger.warn('Rules message not found, creating new one', {
                    guildId,
                    channelId,
        ...
        // Save to database
        const rulesChannel = await this.rulesChannelRepository.upsertByChannelId(guildId, channelId, {
            ...rulesData,
            messageId,
            lastUpdatedBy: updatedBy
        });
        logger_1.logger.info('Rules channel updated', {
            guildId,
            channelId,
            messageId,
            updatedBy
    ...
    async deleteRulesChannel(guildId, channelId) {
        ...
        if (existing.messageId) {
            try {
                ...
                logger_1.logger.warn('Failed to delete rules message', {
                    guildId,
                    channelId,
                    messageId: existing.messageId,
                    error: error instanceof Error ? error.message : 'Unknown error'
        ...
        logger_1.logger.info('Rules channel deleted', {
            guildId,
            channelId,
            deleted: result
        });
    ...
    async syncRulesMessage(guildId, channelId) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to sync rules message', {
                guildId,
                channelId,
                error: error instanceof Error ? error.message : 'Unknown error'
            });
    ...
    async addRule(guildId, channelId, rule, updatedBy) {
    ...
    async removeRule(guildId, channelId, ruleId, updatedBy) {
    ...
    async createNewRulesMessage(channel, embed) {
...
exports.RulesChannelService = RulesChannelService;
//# sourceMappingURL=rules-channel-service.js.map