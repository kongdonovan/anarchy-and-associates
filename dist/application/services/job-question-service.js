...
exports.JobQuestionService = void 0;
const logger_1 = require("../../infrastructure/logger");
class JobQuestionService {
    getQuestionTemplates(category) {
        if (category) {
            return JobQuestionService.QUESTION_TEMPLATES.filter(template => template.category.toLowerCase() === category.toLowerCase());
        }
        return JobQuestionService.QUESTION_TEMPLATES;
    }
    getQuestionCategories() {
        const categories = new Set(JobQuestionService.QUESTION_TEMPLATES.map(template => template.category));
        return Array.from(categories).sort();
    }
    getTemplateById(id) {
        return JobQuestionService.QUESTION_TEMPLATES.find(template => template.id === id) || null;
    }
    createQuestionFromTemplate(templateId, overrides) {
    ...
    validateQuestion(question) {
        try {
            ...
            logger_1.logger.error('Error validating job question:', error);
            return {
                valid: false,
    ...
    validateQuestionList(questions) {
        try {
            ...
            logger_1.logger.error('Error validating question list:', error);
            return {
                valid: false,
    ...
    mergeQuestionsWithDefaults(customQuestions) {
        ...
        const filteredCustomQuestions = customQuestions.filter(custom => !defaultQuestions.some(def => def.id === custom.id));
    ...
    exportQuestionsAsJSON(questions) {
        return JSON.stringify(questions, null, 2);
    }
    importQuestionsFromJSON(jsonString) {
        try {
...
exports.JobQuestionService = JobQuestionService;
JobQuestionService.QUESTION_TEMPLATES = [
    {
        id: 'portfolio_link',
        category: 'Experience',
...
//# sourceMappingURL=job-question-service.js.map