...
describe('FeedbackService Unit Tests', () => {
    ...
    const testFeedbackId = test_utils_1.TestUtils.generateObjectId().toString();
    beforeEach(() => {
        ...
        feedbackService = new feedback_service_1.FeedbackService(mockFeedbackRepository, mockGuildConfigRepository, mockStaffRepository);
        jest.clearAllMocks();
    });
    describe('submitFeedback', () => {
        ...
        it('should submit feedback successfully with all required fields', async () => {
            ...
            mockStaffRepository.findByFilters.mockResolvedValueOnce([test_utils_1.TestUtils.generateMockStaff()]); // Target staff exists
        ...
        it('should submit firm-wide feedback when no target staff specified', async () => {
            ...
            const mockFirmFeedback = test_utils_1.TestUtils.generateMockFeedback({
            ...
            expect(mockFeedbackRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                isForFirm: true,
                targetStaffId: undefined,
                targetStaffUsername: undefined
            }));
        ...
        it('should trim comment whitespace before saving', async () => {
            ...
            mockStaffRepository.findByFilters.mockResolvedValueOnce([test_utils_1.TestUtils.generateMockStaff()]);
            mockFeedbackRepository.add.mockResolvedValue(mockCreatedFeedback);
        ...
        it('should handle all valid rating values', async () => {
            ...
            for (const rating of validRatings) {
                ...
                const mockFeedback = test_utils_1.TestUtils.generateMockFeedback({ rating });
                mockStaffRepository.findByFilters.mockResolvedValueOnce([]); // Submitter is not staff
                mockStaffRepository.findByFilters.mockResolvedValueOnce([test_utils_1.TestUtils.generateMockStaff()]); // Target staff exists
                mockFeedbackRepository.add.mockResolvedValue(mockFeedback);
                const result = await feedbackService.submitFeedback(request);
                expect(result.rating).toBe(rating);
                expect(mockFeedbackRepository.add).toHaveBeenCalledWith(expect.objectContaining({ rating }));
                jest.clearAllMocks();
            }
        });
    ...
    describe('searchFeedback', () => {
        const mockSearchResults = [
            test_utils_1.TestUtils.generateMockFeedback({
                guildId: testGuildId,
                rating: feedback_1.FeedbackRating.FIVE_STAR
            }),
            test_utils_1.TestUtils.generateMockFeedback({
        ...
        it('should search feedback with sort options', async () => {
            ...
            const sort = { field: 'rating', direction: 'desc' };
            mockFeedbackRepository.searchFeedback.mockResolvedValue(mockSearchResults);
            mockFeedbackRepository.countFeedback.mockResolvedValue(2);
        ...
        it('should handle empty search results', async () => {
            const filters = { guildId: 'non-existent' };
        ...
        it('should search by date range', async () => {
            ...
            expect(mockFeedbackRepository.searchFeedback).toHaveBeenCalledWith(expect.objectContaining({
                startDate,
                endDate
            }), undefined, undefined);
        });
    ...
    describe('getStaffPerformanceMetrics', () => {
        const mockStaffMetrics = {
            ...
            ratingDistribution: {
                ...
                [feedback_1.FeedbackRating.TWO_STAR]: 1,
                [feedback_1.FeedbackRating.THREE_STAR]: 2,
                [feedback_1.FeedbackRating.FOUR_STAR]: 4,
                [feedback_1.FeedbackRating.FIVE_STAR]: 3
            },
            recentFeedback: [test_utils_1.TestUtils.generateMockFeedback()]
        };
        ...
        it('should handle repository errors', async () => {
            mockFeedbackRepository.getStaffPerformanceMetrics.mockRejectedValue(new Error('Database connection error'));
    ...
    describe('getFirmPerformanceMetrics', () => {
        const mockFirmMetrics = {
            ...
            firmWideFeedback: [test_utils_1.TestUtils.generateMockFeedback({ isForFirm: true })],
            ratingDistribution: {
                [feedback_1.FeedbackRating.ONE_STAR]: 1,
                [feedback_1.FeedbackRating.TWO_STAR]: 2,
                [feedback_1.FeedbackRating.THREE_STAR]: 7,
                [feedback_1.FeedbackRating.FOUR_STAR]: 10,
                [feedback_1.FeedbackRating.FIVE_STAR]: 5
            }
        };
    ...
    describe('getFeedbackStats', () => {
            ...
            staffMetrics: [
                {
                    ...
                    ratingDistribution: {
                        [feedback_1.FeedbackRating.ONE_STAR]: 0,
                        [feedback_1.FeedbackRating.TWO_STAR]: 0,
                        [feedback_1.FeedbackRating.THREE_STAR]: 1,
                        [feedback_1.FeedbackRating.FOUR_STAR]: 3,
                        [feedback_1.FeedbackRating.FIVE_STAR]: 6
                    },
                    recentFeedback: []
                },
                {
                    ...
                    ratingDistribution: {
                        ...
                        [feedback_1.FeedbackRating.THREE_STAR]: 1,
                        [feedback_1.FeedbackRating.FOUR_STAR]: 3,
                        [feedback_1.FeedbackRating.FIVE_STAR]: 1
                    },
                    recentFeedback: []
                },
                {
                    ...
                    ratingDistribution: {
                        ...
                        [feedback_1.FeedbackRating.THREE_STAR]: 0,
                        [feedback_1.FeedbackRating.FOUR_STAR]: 0,
                        [feedback_1.FeedbackRating.FIVE_STAR]: 2
                    },
                    recentFeedback: []
                }
            ],
        ...
        const mockRecentFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ rating: feedback_1.FeedbackRating.FIVE_STAR }),
            test_utils_1.TestUtils.generateMockFeedback({ rating: feedback_1.FeedbackRating.FOUR_STAR })
        ];
        ...
        it('should sort top rated staff by average rating descending', async () => {
            ...
            const result = await feedbackService.getFeedbackStats(testGuildId);
            expect(result.topRatedStaff).toHaveLength(2);
            expect(result.topRatedStaff[0]?.averageRating).toBeGreaterThanOrEqual(result.topRatedStaff[1]?.averageRating || 0);
        });
        it('should limit top rated staff to 5 entries', async () => {
            const metricsWithManyStaff = {
                ...mockFirmMetrics,
    ...
    describe('getFeedbackById', () => {
        ...
        it('should handle invalid feedback ID format', async () => {
            mockFeedbackRepository.findById.mockRejectedValue(new Error('Invalid ObjectId format'));
    ...
    describe('getFeedbackBySubmitter', () => {
        const mockSubmitterFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ submitterId: testClientId }),
            test_utils_1.TestUtils.generateMockFeedback({ submitterId: testClientId })
        ];
    ...
    describe('getFeedbackByStaff', () => {
        const mockStaffFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ targetStaffId: testStaffId }),
            test_utils_1.TestUtils.generateMockFeedback({ targetStaffId: testStaffId })
        ];
    ...
    describe('getFirmWideFeedback', () => {
        const mockFirmFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ isForFirm: true }),
            test_utils_1.TestUtils.generateMockFeedback({ isForFirm: true })
        ];
    ...
    describe('getRecentFeedback', () => {
        const mockRecentFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ guildId: testGuildId }),
            test_utils_1.TestUtils.generateMockFeedback({ guildId: testGuildId })
        ];
    ...
    describe('calculateRatingTrend', () => {
        ...
        const previousPeriodFeedback = [
            test_utils_1.TestUtils.generateMockFeedback({ rating: feedback_1.FeedbackRating.THREE_STAR }),
            test_utils_1.TestUtils.generateMockFeedback({ rating: feedback_1.FeedbackRating.FOUR_STAR })
        ];
        ...
        it('should calculate stable trend for small changes', async () => {
            ...
            mockFeedbackRepository.searchFeedback
                ...
                .mockResolvedValueOnce([test_utils_1.TestUtils.generateMockFeedback({ rating: feedback_1.FeedbackRating.FOUR_STAR })]); // Previous: avg 4.0
            const result = await feedbackService.calculateRatingTrend(testStaffId, testGuildId, 30);
    ...
    describe('Error Handling', () => {
        it('should handle repository connection failures', async () => {
            mockFeedbackRepository.findById.mockRejectedValue(new Error('Database connection lost'));
        ...
        it('should handle staff repository failures during validation', async () => {
            mockStaffRepository.findByFilters.mockRejectedValue(new Error('Staff service unavailable'));
            const mockRequest = {
                guildId: testGuildId,
                submitterId: testClientId,
                submitterUsername: 'testclient',
        ...
        it('should handle concurrent feedback submission attempts', async () => {
            ...
            mockFeedbackRepository.add.mockRejectedValue(new Error('Duplicate feedback submission detected'));
        ...
        it('should handle search filter validation errors', async () => {
            mockFeedbackRepository.searchFeedback.mockRejectedValue(new Error('Invalid search criteria'));
        ...
        it('should handle performance metrics calculation errors', async () => {
            mockFeedbackRepository.getFirmPerformanceMetrics.mockRejectedValue(new Error('Metrics calculation failed'));
    ...
    describe('Edge Cases', () => {
        it('should handle extremely long feedback comments', async () => {
            ...
            mockStaffRepository.findByFilters.mockResolvedValueOnce([]).mockResolvedValueOnce([test_utils_1.TestUtils.generateMockStaff()]);
            mockFeedbackRepository.add.mockResolvedValue(test_utils_1.TestUtils.generateMockFeedback({ comment: maxLengthComment }));
            const result = await feedbackService.submitFeedback(request);
        ...
        it('should handle special characters in usernames', async () => {
            ...
            mockStaffRepository.findByFilters.mockResolvedValueOnce([]).mockResolvedValueOnce([test_utils_1.TestUtils.generateMockStaff()]);
            mockFeedbackRepository.add.mockResolvedValue(test_utils_1.TestUtils.generateMockFeedback({
                submitterUsername: 'test-client_123!@#',
                targetStaffUsername: 'test-staff_456$%^'
            }));
            const result = await feedbackService.submitFeedback(request);
        ...
        it('should handle empty search results with pagination', async () => {
            const filters = { guildId: 'empty-guild' };
            const pagination = { page: 1, limit: 10 };
            mockFeedbackRepository.searchFeedback.mockResolvedValue([]);
            mockFeedbackRepository.countFeedback.mockResolvedValue(0);
            const result = await feedbackService.searchFeedback(filters, undefined, pagination);
