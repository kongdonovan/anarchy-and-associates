...
describe('Enhanced AuditLogRepository', () => {
    ...
    describe('logGuildOwnerBypass', () => {
        it('should log guild owner bypass with correct structure', async () => {
            ...
            const metadata = { role: staff_role_1.StaffRole.MANAGING_PARTNER, newCount: 2 };
            const result = await auditLogRepository.logGuildOwnerBypass(guildId, actorId, targetId, businessRuleViolated, originalValidationErrors, bypassReason, metadata);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                guildId,
                action: audit_log_1.AuditAction.GUILD_OWNER_BYPASS,
                actorId,
                targetId,
            ...
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Guild owner bypass logged', expect.objectContaining({
                guildId,
                actorId,
                targetId,
                businessRuleViolated,
        ...
        it('should handle missing optional parameters', async () => {
            ...
            const result = await auditLogRepository.logGuildOwnerBypass(guildId, actorId, undefined, // no targetId
            businessRuleViolated, originalValidationErrors
        ...
        it('should handle logging errors gracefully', async () => {
            auditLogRepository.add.mockRejectedValue(new Error('Database error'));
            await expect(auditLogRepository.logGuildOwnerBypass(guildId, actorId, targetId, 'role-limit', ['Error message'])).rejects.toThrow('Database error');
            expect(logger_1.logger.error).toHaveBeenCalledWith('Error logging guild owner bypass:', expect.any(Error));
        });
    });
    describe('logRoleLimitBypass', () => {
        it('should log role limit bypass with detailed information', async () => {
            const role = staff_role_1.StaffRole.MANAGING_PARTNER;
            const currentCount = 1;
            const maxCount = 1;
            const bypassReason = 'Emergency organizational restructure';
            const result = await auditLogRepository.logRoleLimitBypass(guildId, actorId, targetId, role, currentCount, maxCount, bypassReason);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                guildId,
            ...
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Role limit bypass logged', expect.objectContaining({
                guildId,
                actorId,
                targetId,
                role,
        ...
        it('should use default reason when none provided', async () => {
            await auditLogRepository.logRoleLimitBypass(guildId, actorId, targetId, staff_role_1.StaffRole.PARALEGAL, 9, 10
            // no bypassReason
            );
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                details: expect.objectContaining({
    ...
    describe('logBusinessRuleViolation', () => {
        it('should log business rule violations with context', async () => {
            ...
            const action = audit_log_1.AuditAction.CASE_ASSIGNED;
            const metadata = { caseId: 'case_123', attemptedAction: 'view_sensitive_documents' };
            const result = await auditLogRepository.logBusinessRuleViolation(guildId, actorId, ruleViolated, violationDetails, action, targetId, metadata);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                guildId,
            ...
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Business rule violation logged', expect.objectContaining({
                guildId,
                actorId,
                targetId,
                ruleViolated,
    ...
    describe('findGuildOwnerBypasses', () => {
        ...
        it('should handle database errors in bypass search', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith(`Error finding guild owner bypasses for guild ${guildId}:`, expect.any(Error));
        });
    });
    ...
    describe('getBypassStats', () => {
        ...
        it('should handle statistics query errors', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith(`Error getting bypass stats for guild ${guildId}:`, expect.any(Error));
        });
    });
    describe('edge cases and error scenarios', () => {
        it('should handle extremely long bypass reasons', async () => {
            const longReason = 'A'.repeat(10000); // Very long reason
            await auditLogRepository.logRoleLimitBypass(guildId, actorId, targetId, staff_role_1.StaffRole.PARALEGAL, 5, 10, longReason);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                details: expect.objectContaining({
                    reason: longReason
                })
        ...
        it('should handle special characters in metadata', async () => {
            ...
            await auditLogRepository.logGuildOwnerBypass(guildId, actorId, targetId, 'test-rule', ['Test error'], 'Test reason', specialMetadata);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
                details: expect.objectContaining({
        ...
        it('should handle concurrent bypass logging', async () => {
            const promises = Array.from({ length: 5 }, (_, i) => auditLogRepository.logRoleLimitBypass(guildId, `actor_${i}`, `target_${i}`, staff_role_1.StaffRole.PARALEGAL, i, 10, `Reason ${i}`));
        ...
        it('should handle malformed guild IDs gracefully', async () => {
            ...
            await auditLogRepository.logBusinessRuleViolation(malformedGuildId, actorId, 'test-rule', ['Test violation'], audit_log_1.AuditAction.STAFF_HIRED);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should handle array of business rules in violation logging', async () => {
            ...
            await auditLogRepository.logBusinessRuleViolation(guildId, actorId, multipleRules.join(', '), // Multiple rules as single string
            ['Multiple rules violated simultaneously'], audit_log_1.AuditAction.CASE_CREATED);
            expect(auditLogRepository.add).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=enhanced-audit-log-repository.test.js.map