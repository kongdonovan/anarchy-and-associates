...
const error_handler_1 = require("../../infrastructure/error-handling/error-handler");
describe('ApplicationErrorHandler', () => {
    let errorHandler;
    beforeEach(() => {
        errorHandler = error_handler_1.ApplicationErrorHandler.getInstance();
    });
    describe('Error Creation', () => {
        it('should create application errors with all properties', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.VALIDATION_ERROR, 'Invalid input provided', 'Please check your input and try again.', { field: 'username' }, new Error('Original error'));
            expect(error.type).toBe(error_handler_1.ErrorType.VALIDATION_ERROR);
        ...
        it('should create minimal application errors', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.SYSTEM_ERROR, 'System error occurred', 'A system error occurred. Please try again later.');
            expect(error.type).toBe(error_handler_1.ErrorType.SYSTEM_ERROR);
    ...
    describe('Error Handling by Type', () => {
        it('should handle permission denied errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.PERMISSION_DENIED, 'User lacks admin privileges', 'You do not have permission to perform this action.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
        it('should handle validation errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.VALIDATION_ERROR, 'Invalid username format', 'The provided information is not valid. Please check your input and try again.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
        it('should handle resource not found errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.RESOURCE_NOT_FOUND, 'Staff member not found', 'The requested resource could not be found.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
        it('should handle rate limit errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.RATE_LIMIT_EXCEEDED, 'Too many requests', 'You are sending commands too quickly. Please wait a moment.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
        it('should handle database errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.DATABASE_ERROR, 'Database connection failed', 'Our services are temporarily unavailable. Please try again in a few moments.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(false); // System errors are not ephemeral
            expect(result.embed).toBeDefined();
        });
        it('should handle Discord API errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.DISCORD_API_ERROR, 'Discord API returned 503', 'There was an issue communicating with Discord. Please try again.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(false);
            expect(result.embed).toBeDefined();
        });
        it('should handle external service errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.EXTERNAL_SERVICE_ERROR, 'Roblox API unavailable', 'An external service is temporarily unavailable. Please try again later.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(false);
            expect(result.embed).toBeDefined();
        });
        it('should handle queue timeout errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.QUEUE_TIMEOUT, 'Queue operation timed out', 'The operation took too long to complete. Please try again.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
        it('should handle concurrent modification errors correctly', () => {
            const error = errorHandler.createError(error_handler_1.ErrorType.CONCURRENT_MODIFICATION, 'Resource modified by another operation', 'Another operation is currently in progress. Please try again in a moment.');
            const result = errorHandler.handleError(error);
            expect(result.ephemeral).toBe(true);
            expect(result.embed).toBeDefined();
        });
    ...
    describe('Error Execution Wrapper', () => {
        ...
        it('should return error for failed operations', async () => {
            ...
            if (!result.success) {
                expect(result.error.type).toBe(error_handler_1.ErrorType.SYSTEM_ERROR);
        ...
        it('should handle synchronous errors in wrapper', async () => {
            ...
            const result = await errorHandler.executeWithErrorHandling(syncFailingOperation);
            expect(result.success).toBe(false);
            if (!result.success) {
    ...
    describe('Singleton Pattern', () => {
        it('should maintain singleton instance', () => {
            const instance1 = error_handler_1.ApplicationErrorHandler.getInstance();
            const instance2 = error_handler_1.ApplicationErrorHandler.getInstance();
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle non-Error objects', () => {
            ...
            const result = errorHandler.handleError(notAnError);
            expect(result.embed).toBeDefined();
        });
