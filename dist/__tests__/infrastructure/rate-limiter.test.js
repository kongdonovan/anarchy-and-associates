...
describe.skip('RateLimiter', () => {
    ...
    describe('Basic Rate Limiting', () => {
        ...
        it('should allow commands after 1 second interval', async () => {
            ...
            await test_utils_1.TestUtils.wait(1100);
    ...
    describe('Rate Limit Statistics', () => {
        it('should provide accurate statistics', async () => {
            ...
            rateLimiter.checkRateLimit('user1');
            await test_utils_1.TestUtils.wait(100);
            rateLimiter.checkRateLimit('user2');
            await test_utils_1.TestUtils.wait(100);
            rateLimiter.checkRateLimit('user3');
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        it('should maintain singleton instance', () => {
            const instance1 = rate_limiter_1.RateLimiter.getInstance();
            const instance2 = rate_limiter_1.RateLimiter.getInstance();
            expect(instance1).toBe(instance2);
        });
        it('should handle concurrent access from multiple users', async () => {
            const userIds = Array.from({ length: 50 }, (_, i) => `user${i}`);
            // All users make commands simultaneously
            const results = await Promise.all(userIds.map(userId => Promise.resolve(rateLimiter.checkRateLimit(userId))));
            // All first commands should be allowed
            results.forEach(result => {
                expect(result.allowed).toBe(true);
            });
            // All immediate second commands should be blocked
            const secondResults = await Promise.all(userIds.map(userId => Promise.resolve(rateLimiter.checkRateLimit(userId))));
